...
class SoftThresholdOperator(Operator):
    ...
    def __init__(self, domain, base_value, thresholds, values, sharpness):
        ...
        if not len(thresholds) == len(values):
            raise ValueError(
                "`thresholds` {} needs to be same length as "
        ...
        if base_value > values[0]:
            raise ValueError(
                "`base_value` {} cannot be larger than the first "
        ...
        self.delta_values += [
            self.values[i] - self.values[i - 1] for i in range(1, len(self.values))
        ]

        # Generate the k_g values
        self.k_g = [
            self.sharpness[i] / self.delta_values[i] for i in range(len(self.sharpness))
        ]
    ...
    def _call(self, x):
        ...
        for i in range(len(self.k_g)):
            out += (
                self.delta_values[i]
                * 1
                / (1 + self.exp_func[i](x - self.thresholds[i]))
            )
...
class SoftThresholdParamOperator(Operator):
    ...
    def __init__(self, x, base_value, num_thresholds, num_values, num_sharpness):
        """Initialize a new instance.
        ...
        if not num_thresholds == num_values:
            raise ValueError(
                "`num_thresholds` {} must be the same as "
                "`num_values` {}".format(num_thresholds, num_values)
            )

        if not num_thresholds == num_sharpness:
            raise ValueError(
                "`num_thresholds` {} must be the same as "
                "`num_sharpness` {}".format(num_thresholds, num_sharpness)
            )
    ...
    def _call(self, x):
        ...
        delta_values += [values[i] - values[i - 1] for i in range(1, self.num_values)]

        # Generate the k_g values
        k_g = [sharpness[i] / delta_values[i] for i in range(self.num_sharpness)]
        ...
        for i in range(len(k_g)):
            out += (
                delta_values[i] * 1 / (1 + self.exp_fun(self.x - thresholds[i], k_g[i]))
            )
    ...
    def derivative(self, point):
        ...
        ops = (-2.0 * sharpness[0] * tmp / (1 + tmp) ** 2) * proj(self.domain, 0)

        for i in range(1, self.num_thresholds):
            tmp = self.exp_fun(np.abs(self.x - thresholds[i]), k_g[i])
            ops += (-2.0 * sharpness[i] * tmp / (1 + tmp) ** 2) * proj(self.domain, i)
        ...
        for i in range(self.num_sharpness):
            tmp = self.exp_fun(np.abs(self.x - thresholds[i]), k_g[i])
            ops += (2 * (self.x - thresholds[i]) * tmp / (1 + tmp) ** 2) * proj(
                self.domain, self.num_thresholds + self.num_values + i
            )
...
class SoftThresholdReducedParamOperator(Operator):
    ...
    def _call(self, x):
        ...
        # Generate the k_g values
        k_g = [self.sharpness[i] / delta_values[i] for i in range(self.num_values)]
    ...
    def derivative(self, point):
        ...
        err_backup = np.seterr(over="ignore", under="ignore")
        tmp = self.exp_fun(np.abs(self.x - thresholds[0]), k_g[0])
        ops = (-2.0 * self.sharpness[0] * tmp / (1 + tmp) ** 2) * proj(self.domain, 0)

        for i in range(1, self.num_thresholds):
            tmp = self.exp_fun(np.abs(self.x - thresholds[i]), k_g[i])
            ops += (-2.0 * self.sharpness[i] * tmp / (1 + tmp) ** 2) * proj(
                self.domain, i
            )
...
class ThresholdOperator(Operator):
    ...
    def __init__(self, domain, thresholds, values):
        ...
        if not len(thresholds) + 1 == len(values):
            raise ValueError(
                "`thresholds` {} needs to be one shorter than "
                "`values` {}".format(thresholds, values)
            )
    ...
    def thresholding(self, x):
        ...
        thresholded = np.full_like(x.asarray(), self.values[0])
        for i, t in enumerate(self.thresholds):
            thresholded[x.asarray() > t] = self.values[i + 1]
        return thresholded

        # for i in range(len(self.thresholds)):
        #     if x.asarray() < self.thresholds[i]:
...
class EdgeDetectOperator(Operator):
    ...
    def __init__(self, domain, diagonal_neighbour=False):
        ...
        self.forward_grad = Gradient(domain=domain, method="forward", pad_mode="order0")
        self.backward_grad = Gradient(
            domain=domain, method="backward", pad_mode="order0"
        )
    ...
    def _call(self, x):
        ...
        if not self.diagonal_neighbour:
            tmp = (
                self.forward_grad(x).ufuncs.absolute()
